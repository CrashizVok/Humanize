...
Run this **after** the structural pass in `narrative-structure.md`, never instead of it. A
professional-grade surface edit moves a narrative-feature detector by 1.6 points; that is the
measured ceiling on what this layer can do alone.

The 35 numbered patterns below come from Wikipedia's
["Signs of AI writing"](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing), maintained
by WikiProject AI Cleanup, by way of the vendored blader/humanizer skill in
`.claude/skills/humanizer/` (MIT). That directory is a third-party package with its own version
history and validation script; do not edit it. Edit this file instead.

Wikipedia's framing, which is the reason any of these patterns exist: "LLMs use statistical
algorithms to guess what should come next. The result tends toward the most statistically likely
...
Patterns 1–35 are about what the text says. These are about how it moves. They come from
`marketing/prompts/blog-humanize-pass2.md`, which was written for ZeroHook blog drafts and
generalizes to anything longer than a few paragraphs.

### 36. Even sentence length

**Problem:** AI prose settles into a mid-length cadence and stays there. Human writing swings.
**Rule:** every section needs at least one sentence under 8 words and at least one over 25.
Break any run of four or more sentences with similar word counts.
**Before:**
> The scan checks the SPF record first. It then validates the DKIM selectors in turn. After that
> it reads the DMARC policy from DNS. Each result is stored with its own verdict.
**After:**
> The scan reads SPF first, then walks each DKIM selector, then pulls the DMARC policy. Four
> lookups, one verdict each. If any of them times out the check returns Info rather than Pass,
> because a green tick that rests on a failed lookup is a claim the customer has to defend to
> their auditor.

### 37. Repeated paragraph shape

**Problem:** several paragraphs in a row with the same internal shape, most often
statistic, then explanation, then implication.
**Rule:** no two consecutive paragraphs with the same shape. Delete the neat closing sentence
from at least two paragraphs in a long piece and let them end mid-thought.

### 38. Uniform list items

**Problem:** every bullet the same length, every numbered step one sentence, every FAQ answer
one paragraph.
**Rule:** vary them. One step is a line, the next is three sentences. One answer is a sentence,
the next is a list. If every item is the same size, the list was generated rather than written.

### 39. Subject monotony

**Problem:** a long run of sentences that all open with the same subject, most often the product
or company name.
**Rule:** if more than half the sentences in a section start with the same subject, rewrite half
of them into "you" voice or fold them into the sentence before.

### 41. A true list that gains a false member

**Problem:** three real items and a fourth that arrived because the sentence wanted a fourth.
Pattern 10 bans forcing ideas into threes for rhythm; this is the factual version of the same
pressure, and it is far more dangerous, because every neighbouring item checks out and carries
the invented one past review.

**Before:**
> Every finding ships with the exact record to paste, the host, the TTL, and where that field
> lives in Cloudflare, Route 53, GoDaddy, or Namecheap.
**After:**
> Every finding ships with the exact record to paste, its host, and where that field lives in
> your DNS provider's panel.

The record, the host and the provider are real fields on the fix object. The TTL is not; it came
along because a list of three reads better than a list of two. Nobody checking that sentence
would stop at the third item after the first two were right.

**The test:** point at each item in a list separately, and name where that one comes from. Not
the list — the item. An item you can only justify as "well, the others are right" is the one to
cut. Where the real list is two items long, write two.

### 40. Fragments that do not expand into a claim

**Problem:** a phrase that is grammatical, true and says nothing a reader can restate. It usually
appears where a character limit squeezes a second idea in beside the first: taglines, meta
descriptions, subject lines, slide titles, feature bullets.

**The test:** read the clause on its own and expand it into a full sentence without guessing what
was meant. "No card required" expands to "no card is required", which is a claim. "No enterprise
price" expands to "there is no enterprise price", which is not what it means and is not true
either; the reader has to reconstruct "it does not cost what enterprise tools cost" from
context that is not there.

**Before:**
> NIS2-ready DNS & email security audits, no enterprise price
**After:**
> NIS2-ready DNS and email security audits, from $0

**Why this pattern is its own item:** everything else in this file is about text that sounds
generated, and everything in `narrative-structure.md` is about text that is shaped wrong. This is
text that passes both and still means nothing. It survives a factual review too, because there is
nothing in it to be wrong about.

**Fit a limit by cutting a whole idea, not by compressing a claim into a fragment.** One complete
thought inside 60 characters beats two halves that each need the other to make sense.

---

## Language note

Pattern §14 (em and en dashes) is **English-only**. In Hungarian the gondolatjel (–) is correct
punctuation, not an AI tell, and the quotation marks are „ ” rather than straight quotes, which
reverses §19 as well. See `hungarian.md` before applying either rule to Hungarian text.
